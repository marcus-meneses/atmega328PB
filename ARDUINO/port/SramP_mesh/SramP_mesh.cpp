...
Sram::Sram(SPIClass *periph, uint8_t CS) {
    	peripherial = periph;
		SPI_cs = CS;
		release();
...
void    Sram::blockCopy(uint32_t startaddr, uint32_t endaddr, uint32_t size){
        
	for (uint32_t cursor=0; cursor<size; cursor++){
	        memwrite(endaddr+cursor,memread(startaddr+cursor));
		//Serial.print( memread(endaddr+cursor));
		//Serial.print(F(" "));
	}
...
void    Sram::blockLoad(uint32_t startaddr, uint8_t * data , uint32_t size) {
	startSerialRead(startaddr);
	//Serial.println();Serial.print(F("BLD "));
	//Serial.println();//Serial.print(F("BLD  @"));//Serial.print(startaddr);//Serial.print(F("   "));
	for (uint32_t cursor=0; cursor<size; cursor++){
	          data[cursor] = byteout();
		  //Serial.write(data[cursor]);
	}
	endSerialRead();
}

//sends sized  array to memory block
void    Sram::loadBlock(uint8_t * data, uint32_t startaddr , uint32_t size) {
	startSerialWrite(startaddr);
	//Serial.println();//Serial.print(F("LBD  @"));//Serial.print(startaddr);//Serial.print(F("   "));
	for (uint32_t cursor=0; cursor<size; cursor++){
	          bytein(data[cursor]);
		  //Serial.write(data[cursor]);
	}
...
void    Sram::blockErase(uint32_t startaddr, uint32_t size){
